pub mod dispatch{
    use bevy::{ecs::{system::{Resource, Commands, Res, ResMut, Query}, entity::Entity, event::{EventWriter, EventReader}}, math::Vec2, app::{Plugin, App, Startup, Update}, asset::{AssetServer, Assets}, sprite::{TextureAtlas, SpriteSheetBundle}, transform::components::Transform, prelude::default};

    use crate::{TILES_WIDTH, StreamEvent, StreamReceiver, map::map::spawn_map, Ressource::Ressource::{Ressource, spawn_resources}, sprite_player::{sprite_player::{setup_sprite, ActionPlayer, TypeAction, add_action, SpriteAnimation, SpriteComponent, set_sprite_animation, Cell}, self}};

    // const for teams folder name 
    pub const VECSPRITE: [&'static str; 2] = ["yoshi_green_up.png", "yoshi_walking3.png"]; 
    ...
    impl RessCommandId{
        ...
        pub fn set_all_sprites(& mut self, mut texture_atlases: & mut ResMut<Assets<TextureAtlas>>, asset_server: &Res<AssetServer>) //(nbr_teams)
        {
            ...
            {
                println!("{}", i);
                self.vec_sprite_player.push(set_sprite_animation(0, i, texture_atlases, asset_server))
    ...
    {
        commands.insert_resource(RessCommandId{x: 0, y: 0, id_Ressource: vec![], player_id: vec![], vec_sprite_player: vec![]});
